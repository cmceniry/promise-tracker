...
use crate::components::{Behavior, Item, Pattern};
use std::fmt;
...
pub fn check_items(items: &[Item]) -> Vec<SemanticError> {
    ...
    for item in items {
        ...
        match item {
            ...
            Item::SuperAgent(superagent) => {
                for (i, instance) in superagent.get_instances().iter().enumerate() {
                    check_behaviors(
                        &name,
                        &format!("instances[{}].", i),
...
fn check_behaviors(
    ...
    for (i, want) in wants.iter().enumerate() {
        ...
        // Restriction A: resolution always starts from a concrete goal.
        if !want.get_name_pattern().is_ground() {
            errors.push(SemanticError {
                item: item.to_string(),
                location: at,
                name: want.get_name().clone(),
                message: String::from(
                    "a want names one concrete behavior; variables belong to whoever provides it",
                ),
            });
        }
    }
...
",
        ...
        assert_eq!(found.len(), 1);
        assert!(found[0].message.contains("adjacent"), "{}", found[0].message);
    }
...
",
    ...
    #[test]
    fn a_want_may_not_be_parameterized() {
        let found = check(
            "kind: Agent
name: p1
wants:
...
",
        ...
        assert_eq!(found.len(), 1);
        assert_eq!(found[0].location, "wants[0]");
        assert!(
            found[0].message.contains("one concrete behavior"),
            "{}",
            found[0].message
        );
    }
    ...
    #[test]
    fn superagent_instances_are_checked_too() {
        let found = messages(
            "kind: SuperAgent
name: sa1
instances:
...
",
        ...
        assert_eq!(found.len(), 1);
        assert!(
            found[0].starts_with("SuperAgent/sa1 instances[0].wants[0]:"),
            "{}",
            found[0]
        );
    }
